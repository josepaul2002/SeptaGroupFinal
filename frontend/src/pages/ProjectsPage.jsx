...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    ...
    setFiltered(result);
  }, [typeFilter, statusFilter, projects]);
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-10 lg:px-12">
          <div className="max-w-2xl">
            ...
            <p className="text-base md:text-lg font-inter font-light text-[#1F2328]/55 leading-relaxed reveal reveal-delay-2">
              A selection of institutional, commercial, healthcare, and residential projects delivered across Kerala. Each project entry shows the challenge we solved, not just the building we finished.
            </p>
          </div>
        </div>
      </section>
            ...
            <div className="flex flex-wrap gap-2">
              {typeFilters.map(f => (
                <button
                  ...
                  data-testid={`type-filter-${f.toLowerCase()}`}
                  className={`text-xs font-inter px-3 py-1.5 border transition-colors ${
                    typeFilter === f
                ...
                <button
                  ...
                  className={`text-xs font-inter px-3 py-1.5 border transition-colors ${
                    statusFilter === f
                      ...
                      : 'text-[#1F2328]/60 border-[#A7ADB5]/30 hover:border-[#1F2328] hover:text-[#1F2328]'
                  }`}
                >
                  {f}
                </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3, 4, 5, 6].map(i => (
                <div key={i} className="animate-pulse">
                  <div className="aspect-[4/3] bg-[#E8E6E0] mb-4" />
                  <div className="h-4 bg-[#E8E6E0] w-3/4 mb-2" />
                  <div className="h-3 bg-[#E8E6E0] w-1/2" />
                </div>
              ))}
            </div>
          ) : filtered.length === 0 ? (
