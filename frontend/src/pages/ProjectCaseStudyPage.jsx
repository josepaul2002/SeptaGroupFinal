...
import { ArrowLeft, ArrowRight, Check, MapPin, Calendar, Maximize2, Building2, Users } from 'lucide-react';
...
function getRelatedProjects(current) {
  return projects
    .filter(p => p.slug !== current.slug && (
      p.clientLens === current.clientLens ||
      p.partnerStack?.some(ps => current.partnerStack?.some(cp => cp.partnerId === ps.partnerId)) ||
      p.design?.designTags?.some(t => current.design?.designTags?.includes(t))
    ))
    .slice(0, 3);
}
...
export default function ProjectCaseStudyPage() {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    if (project) document.title = `${project.title} — Septa Group`;
  }, [project]);
  ...
  const labels = lensLabels[project.clientLens] || lensLabels.Commercial;
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-10 lg:px-12">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
            ...
            <div className="md:col-span-9">
              ...
              {project.partnerStack?.map(ps => {
                const partner = getPartner(ps.partnerId);
                return (
                  <div key={ps.partnerId} className="flex items-start gap-4 py-3.5 border-b border-[#F3F0E8]/10"
                    data-testid={`stack-row-${ps.partnerId}`}>
                    ...
                    <div className="flex-1">
                      {partner ? (
                        ...
                        <span className="text-sm font-inter font-medium text-[#F3F0E8]">{ps.partnerId}</span>
                      )}
                      <p className="text-xs font-inter font-light text-[#F3F0E8]/45 mt-0.5 leading-relaxed">{ps.contribution}</p>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-10 lg:px-12">
          <div className="grid grid-cols-2 md:grid-cols-6 gap-6">
            {[
              ...
              { icon: <Building2 size={12} strokeWidth={1.5} />, label: 'Client Type', val: project.clientType },
              { icon: <Users size={12} strokeWidth={1.5} />, label: 'Lens', val: project.clientLens },
              { icon: null, label: 'Status', val: project.status },
            ].map((m, i) => (
              <div key={i}>
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-10 lg:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-9 space-y-5 reveal reveal-delay-1">
              {project.story?.storyParagraphs?.map((para, i) => (
                <p key={i} className="text-base font-inter font-light text-[#1F2328]/65 leading-relaxed">{para}</p>
              ))}
              {project.story?.ownerQuote && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            <div className="lg:col-span-3 reveal">
              ...
              <div className="flex flex-wrap gap-1.5 mt-5">
                {project.design?.designTags?.map(tag => (
            ...
            <div className="lg:col-span-9 space-y-10 reveal reveal-delay-1">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {project.delivery.deliveryHighlights.map((h, i) => (
                      <div key={i} className="flex items-start gap-3 p-4 bg-white border border-[#1F2328]/8">
                        ...
                        <p className="text-sm font-inter font-light text-[#1F2328]/65 leading-relaxed mt-0.5">{h}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 gap-6">
                <div>
                  ...
                  <p className="text-sm font-inter font-medium text-[#1F2328] mb-2">{project.challenge}</p>
                  <p className="text-sm font-inter font-light text-[#1F2328]/60 leading-relaxed">{project.challengeDetail}</p>
                </div>
                <div>
                  <div className="flex items-center gap-3 mb-2">
                  ...
                  <p className="text-sm font-inter font-light text-[#1F2328]/60 leading-relaxed">{project.approachDetail}</p>
                </div>
                <div>
                  <div className="flex items-center gap-3 mb-2">
                  ...
                  <p className="text-sm font-inter font-light text-[#1F2328]/60 leading-relaxed">{project.outcomeDetail}</p>
                </div>
              </div>
            </div>
          </div>
              ...
              {relatedProjects.map((rp, i) => (
                <Link key={rp.slug} to={`/projects/${rp.slug}`}
                  ...
                  <div className="flex items-center justify-between mb-1">
                    <h3 className="text-sm font-sora font-medium text-[#1F2328]">{rp.title}</h3>
