...
const statusColors = {
  ...
  contacted: 'bg-[#ECECEA] text-[#666666]',
  qualified: 'bg-[#050505] text-white',
  closed: 'bg-[#ECECEA] text-[#666666]',
};
  ...
  return (
    <div className="bg-white border border-[#8A8A8A]/20 p-6" data-testid="leads-tab">
      ...
      {loading ? (
        ...
        <p className="text-sm text-[#8A8A8A] py-8 text-center">No leads found</p>
      ) : (
            ...
            <thead>
              <tr className="border-b border-[#8A8A8A]/20">
                {['Name', 'Contact', 'Project', 'Budget', 'Status', 'Date', ''].map(h => (
                  <th key={h} className={`${h ? 'text-left' : 'text-right'} py-3 px-2 font-inter font-medium text-[#8A8A8A] text-xs uppercase tracking-wider`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {leads.map(lead => (
                <tr key={lead.id} className="border-b border-[#8A8A8A]/10 hover:bg-[#F6F6F3]/50" data-testid={`lead-row-${lead.id}`}>
                  ...
                  <td className="py-3 px-2">
                    <p className="font-inter text-[#606060]">{lead.phone}</p>
                    {lead.email && <p className="text-xs text-[#8A8A8A]">{lead.email}</p>}
                  </td>
                  <td className="py-3 px-2">
                    <p className="font-inter text-[#050505]">{lead.project_type || '-'}</p>
                    <p className="text-xs text-[#8A8A8A]">{lead.project_location || '-'}</p>
                  </td>
                  <td className="py-3 px-2">
                    <p className="text-xs text-[#050505]">{lead.budget_range || '-'}</p>
        ...
        <div className="flex items-center justify-between mt-4 pt-3 border-t border-[#8A8A8A]/10">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1}
              className="flex items-center gap-1 px-3 py-1 text-xs border border-[#8A8A8A]/30 disabled:opacity-30" data-testid="leads-next">
