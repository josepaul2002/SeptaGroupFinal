  ...
  return (
    <div className="space-y-4" data-testid="page-content-tab">
        ...
        {['about', 'services'].map(p => (
          <button key={p} onClick={() => setActivePage(p)} data-testid={`page-tab-${p}`}
            className={`px-4 py-2 text-xs font-inter uppercase tracking-wider border-b-2 transition-colors ${
              activePage === p ? 'text-[#0F5E5B] border-[#0F5E5B] font-medium' : 'text-[#A7ADB5] border-transparent'
            }`}>{p}</button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <>
          {blockTypes.map(bt => (
            <div key={bt.type} className="bg-white border border-[#A7ADB5]/20 p-5" data-testid={`block-group-${bt.type}`}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-sora font-medium text-[#1F2328]">{bt.label}s ({groupedBlocks[bt.type]?.length || 0})</h3>
                <button onClick={() => addBlock(bt.type)} data-testid={`add-${bt.type}`}
                  className="flex items-center gap-1 px-3 py-1 text-xs font-inter text-[#0F5E5B] border border-[#0F5E5B]/30 hover:bg-[#0F5E5B] hover:text-white transition-colors">
                  <Plus size={12} /> Add
                </button>
              </div>
              <div className="space-y-3">
                {(groupedBlocks[bt.type] || []).map((block, i) => (
                  <div key={block.id} className="border border-[#A7ADB5]/15 p-4 hover:border-[#A7ADB5]/30 transition-colors">
                    <div className="flex items-center gap-2 mb-3">
                      <GripVertical size={12} className="text-[#A7ADB5]" />
                      <span className="text-[10px] text-[#A7ADB5] uppercase tracking-wider">{bt.label} #{i + 1}</span>
                      <div className="ml-auto flex gap-1">
                        <button onClick={() => moveBlock(block.id, -1)} className="p-1 text-[#A7ADB5] hover:text-[#1F2328]"><ChevronUp size={12} /></button>
                        <button onClick={() => moveBlock(block.id, 1)} className="p-1 text-[#A7ADB5] hover:text-[#1F2328]"><ChevronDown size={12} /></button>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {bt.fields.includes('title') && (
                        <div>
                          <label className="text-[10px] text-[#A7ADB5] uppercase tracking-wider block mb-1">Title (EN)</label>
                      ...
                      {bt.fields.includes('subtitle') && (
                        <div>
                          <label className="text-[10px] text-[#A7ADB5] uppercase tracking-wider block mb-1">Subtitle (EN)</label>
                      ...
                      {bt.fields.includes('body') && (
                        <div className="md:col-span-2">
                          <label className="text-[10px] text-[#A7ADB5] uppercase tracking-wider block mb-1">Body (EN)</label>
                      ...
                      {bt.fields.includes('icon') && (
                        <div>
                          <label className="text-[10px] text-[#A7ADB5] uppercase tracking-wider block mb-1">Icon</label>
                      ...
                      {bt.fields.includes('image_url') && (
                        <div>
                          <label className="text-[10px] text-[#A7ADB5] uppercase tracking-wider block mb-1">Image URL</label>
                      ...
                      {bt.fields.includes('link_url') && (
                        <div>
                          <label className="text-[10px] text-[#A7ADB5] uppercase tracking-wider block mb-1">Link URL</label>
                      ...
                      {bt.fields.includes('link_label') && (
                        <div>
                          <label className="text-[10px] text-[#A7ADB5] uppercase tracking-wider block mb-1">Link Label</label>
                      ...
                      {bt.fields.includes('metadata.traditional') && (
                        <div className="md:col-span-2">
                          <label className="text-[10px] text-[#A7ADB5] uppercase tracking-wider block mb-1">Traditional (comparison column)</label>
