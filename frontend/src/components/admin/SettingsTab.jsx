    ...
    <div className="space-y-6" data-testid="settings-tab">
        ...
        {sections.map(s => (
          <button key={s.id} onClick={() => setActiveSection(s.id)}
            ...
            className={`px-4 py-2.5 text-xs font-inter uppercase tracking-wider border-b-2 transition-colors ${
              activeSection === s.id ? 'text-[#606060] border-[#606060] font-medium' : 'text-[#8A8A8A] border-transparent hover:text-[#050505]'
            }`}>{s.label}</button>
        ))}
      </div>
      ...
      {activeSection === 'contact' && (
        <div className="bg-white border border-[#8A8A8A]/20 p-6 space-y-4">
          ...
          <div>
            <label className="text-xs uppercase tracking-widest text-[#050505]/50 font-inter block mb-2">Operating Districts (one per line)</label>
      ...
      {activeSection === 'enquiry' && (
        <div className="bg-white border border-[#8A8A8A]/20 p-6 space-y-4">
          ...
          <div>
            <label className="text-xs uppercase tracking-widest text-[#050505]/50 font-inter block mb-2">Budget Ranges (one per line)</label>
          ...
          <div>
            <label className="text-xs uppercase tracking-widest text-[#050505]/50 font-inter block mb-2">Timeline Ranges (one per line)</label>
          ...
          {['english_only', 'malayalam_primary', 'toggle'].map(mode => (
            <label key={mode} className={`flex items-start gap-3 p-3 border cursor-pointer transition-colors ${
              ...
              <input type="radio" name="lang_mode" checked={settings.content_language_mode === mode}
                ...
                className="mt-0.5" />
              <div>
                <p className="text-sm font-inter font-medium text-[#050505]">
      ...
      {activeSection === 'export' && (
        <div className="bg-white border border-[#8A8A8A]/20 p-6 space-y-6">
          <div>
            <h3 className="text-sm font-sora font-medium text-[#050505] mb-2">Export Content</h3>
            <div className="flex gap-3">
              <button onClick={handleExport} disabled={exporting} data-testid="export-json-btn"
                className="flex items-center gap-2 px-4 py-2 bg-[#050505] text-white text-xs font-inter font-medium uppercase tracking-wider hover:bg-[#262626] transition-colors disabled:opacity-60">
              ...
              <button onClick={handleExportCSV} data-testid="export-csv-btn"
                className="flex items-center gap-2 px-4 py-2 border border-[#606060] text-[#606060] text-xs font-inter font-medium uppercase tracking-wider hover:bg-[#050505] hover:text-white transition-colors">
